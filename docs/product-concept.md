...
Feedback captured there: use Nextcloud tags for lightweight cross-app labels, but make stars, Library-owned reading activity, workflow status, and structured genres/classifications canonical Library-native fields when they need first-class filters, export/import or File-First sidecar portability. Personal starring and last-opened activity are now implemented as Library-native catalogue state.
