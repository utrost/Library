...
Current status: `0.1.0-alpha.173` unsigned package rehearsal complete for the current source candidate. Manual AT testing is pending; accessibility-tree evidence is not screen-reader testing.
Current candidate baseline: `0.1.0-alpha.173`
Future authorized signing uses `openssl dgst -sha512 -sign ~/.nextcloud/certificates/library.key dist/library-0.1.0-alpha.173.tar.gz`; no alpha.171 signed archive exists yet.
