...
Alpha.169 status: `0.1.0-alpha.170` source candidate, unpackaged. Manual AT testing is pending; accessibility-tree evidence is not screen-reader testing.
Current candidate baseline: `0.1.0-alpha.170`
Future authorized signing uses `openssl dgst -sha512 -sign ~/.nextcloud/certificates/library.key dist/library-0.1.0-alpha.170.tar.gz`; no alpha.169 archive exists yet.
