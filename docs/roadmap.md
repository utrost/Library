...
- Scan/admin controls remain on the personal settings surface at `/settings/user/library`. The app remains catalogue-first and preserves its absolute Nextcloud URLs, generated for the active webroot. Alpha.160 introduced the native `@nextcloud/vue` application-shell scaffold, alpha.161 made Review a focused native destination, and alpha.162 turns the sidebar into a read-only catalogue/Review item detail using a canonical `item` query parameter and a narrow user-scoped endpoint. The PHP full-details page remains the editing and fallback surface; Settings migration remains deferred.
...
Feedback: Nextcloud tags are useful as cross-app labels, but stars, Library-owned reading activity, workflow status, and genres/classifications should become canonical Library-native fields when they affect catalogue UX, filters, export/import or external metadata-tool handoff.
...
2. **P1 — Metadata correction workflow.** Details editing works, but one user edit still protects the whole item from rescan overwrite. The P1 field-level provenance foundation is now started: scanner field sources and scanner candidate values are stored as JSON maps and displayed on the details page while preserving item-level `user_edited` protection. Manual edits preserve stored scanner candidates. Rescans refresh scanner candidates for user-edited items without overwriting user-edited fields. The single-field reset-to-scanner path has landed so individual fields can be restored from stored scanner candidates without dropping the whole user-edited item. Whole-item reset to scanner candidates has landed for applying all stored candidates at once while keeping the item row user-edited. Non-blocking edit guidance has landed for date, language and creator fields; validation guidance has landed as hints, and first hard validation for publication dates and language codes has landed. Field-level conflict visibility has landed as a details-table label when current values differ from scanner candidates. Metadata correction summary has landed as read-only counts for scanner candidates and fields differing from scanner. Conflict review filter has landed as the first catalogue-level review workflow. A batch metadata reset now applies stored scanner candidates to current scanner-conflict results selected by catalogue filters; arbitrary bulk edit and richer validation remain future work.
3. **P2 — Discovery by publication structure.** Series and periodicals UX is the next common-use priority. Search/filter/pagination are implemented, and the catalogue now supports filtering and sorting by series or periodical title through the existing publication field. A top series and periodicals panel now lists item counts and links to the dedicated publication discovery page, with empty-state guidance when no series metadata exists. The publication discovery page now includes **Publication contents** read-only issue/date grouping: stable issue/date order, year/month buckets, conservative volume grouping from title/subtitle/path candidates, gap hints for missing issue numbers, and an **Unknown issue/date** bucket so weak rows stay visible. The first dedicated publication year discovery page has landed as a named context around the existing compact catalogue grid for that year. The creator discovery page has landed as the first exact full-field creator landing page, with richer creator identity splitting still future work. The publication year filter has landed as read-only discovery for dated books, magazines, journals and comics. Creator filtering has landed as exact full-field matching for read-only discovery; creator identity splitting remains future work. Active filter chips have landed as read-only navigation so users can remove one filter without clearing the whole search. Compact cover-first cards have landed for mobile and desktop so browsing prioritizes covers and keeps secondary metadata/actions behind a touch disclosure. Built-in useful views now cover daily destinations and cleanup queues: recently opened, starred, workflow statuses, needs metadata, scanner conflicts, metadata errors, likely placeholder covers, no creator, no publication/series, missing date, filename-derived title, weak filename/path metadata, missing description, unsupported archive/container candidates and unreviewed imports. The weak-metadata cockpit shows those sparse/suspicious metadata counts as stable links back to normal catalogue filters, so cleanup starts from indexed metadata and provenance instead of manual lists. In-app custom collections now let users save arbitrary current catalogue filters as named collections with count badges. Their normal counts now avoid row and facet fetches; runtime PHP database coverage for saved raw-tag filters and an SQL-only scanner-conflict count remain pending. Richer issue grouping polish, issue-level metadata editing, next/previous issue routes and creator identity splitting remain future work.
...
7. **P6 — Root/onboarding/shared-library polish.** Root lifecycle works, typed root-delete confirmation is in place for destructive app-row cleanup, recovery copy now explains export/backup/rescan consequences before deleting a root, and first-run/empty-state guidance explains the add-root → scan → browse path. Remaining release polish is richer validation and eventually admin-managed shared-root provisioning.

Tag result feedback has landed, and one-click suggested tag buttons have landed for the existing detail-page Nextcloud tag workflow: after add attempts, Library reports added, already assigned, empty and not-assignable outcomes while keeping catalogue cards browse-only and tags separate from Library-native genres/classifications. The picker only offers currently unassigned visible/assignable tags. Filter-result batch tagging now applies or removes a Nextcloud tag from current filter results up to the safety cap; batch tag removal reports requested/removed/not-tagged/skipped counts. Batch metadata reset has landed for current scanner-conflict results selected by catalogue filters. Batch cover refresh has landed as a read-only no-store retry for current filtered cover URLs. Richer batch metadata changes, app-owned batch cover cache/rebuild and bulk taxonomy workflows remain future work.
...
- **Item metadata update:** present. The details page owns publication metadata editing and user-edited items are preserved across rescans. Batch metadata apply now supports a preview-and-apply workflow for one selected field across current filter results: the preview page estimates the blast radius, then the apply route writes changed rows and reports requested/applied/unchanged/skipped counts.
...
1. **Root lifecycle UI and routes.** First slice landed: per-root edit label/path, enable/disable and delete actions in `/settings/user/library`; source files are untouched, settings copy explains that delete affects Library catalogue/index data, not Nextcloud Files, and typed root-delete confirmation now blocks accidental one-click deletion. Remaining polish: validation and clearer recovery story.
...
4. **Root removal policy.** First slice landed: root deletion uses the conservative v0.1 behaviour, removing app-owned root/index/catalogue rows only, asks users to type `DELETE`, avoids deleting source files from Library, and shows clearer recovery copy before deletion.
...
10. **Metadata portability.** Metadata export foundation.** First slice landed: user-edited catalogue rows can be downloaded as side-effect-free JSON with stable file identity, root/shelf labels, publication metadata and provenance. Write-back to OPF or JSON sidecars is intentionally outside the app roadmap and belongs to external file-first tooling.
11. **Catalogue scale follow-ups.** Count-only badges, projected catalogue DTOs, workload-led indexes and the conservative unchanged-file metadata fast path have landed. Ordinary trusted indexed files with existing items/current pipeline revision can skip extraction and item writes when the primary plus selected OPF fingerprint is unchanged; weak signals and all changed/error/repair cases extract. Historical alpha.153 exact-package smoke measured write elision on a privacy-safe 40-file root: warm-up rewrote 40 item rows and established 40 markers, while the unchanged second scan rewrote zero item rows and retained all 40 markers. Alpha.157 exact-package smoke independently ran the same 40-file root twice with `indexed=40`, `missing=0`, `errors=0`, zero catalogue rewrites, 40 markers and `source_observation_changes=0` on both runs. Exact-package install, Vue/API and browser evidence passed, with zero browser console errors and `release_package_smoke_ok=true`. This does not instrument source writes or compare content bytes, and it is not throughput or latency evidence. Pipeline revision must be bumped for every output-affecting extractor, normalization, sidecar precedence, filename/folder interpretation or ItemService candidate-mapping change. Alpha.154 aggregate instrumentation has landed. Because alpha.157 reported `No upgrade required`, a fresh database migration rehearsal remains pending, as do duplicated creator URL cleanup, lazy descriptions, the saved raw-tag fix and SQL-native scanner-conflict counting; no elapsed-speedup claim is made.
...
6. Keep metadata export/import as the in-app portability boundary; source-folder OPF/JSON sidecar writing and full file-neighbour restore belong to external file-first tooling, while the current import/apply flow only writes matched existing Library catalogue rows.
