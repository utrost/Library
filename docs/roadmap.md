...
3. **P2 — Metadata portability.** Corrected metadata export exists, and **Metadata import preview.** First slice landed: a read-only preview endpoint can parse a Library corrected metadata JSON export and report matched/missing items plus differing fields. For now, apply imported metadata remains future work, as does OPF/JSON sidecar write-back so corrections become File-First durable.
