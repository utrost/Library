...
5. **P4 — Cover quality path.** Preview/CBZ/placeholder covers are good enough for browsing, first EPUB cover extraction from package manifests has landed, the per-detail cover refresh affordance can retry the cover route with no-store response headers while the detail page keeps the cover-source explanation on the refresh action, manual cover override/revert can fix individual bad covers from a URL or upload, and batch cover refresh can request fresh no-store cover previews for current catalogue filter results. Missing pieces are an app-owned cover cache and crop/rebuild workflows.
