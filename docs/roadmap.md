...
Filename and folder names remain searchable for sparse PDFs/comics, alongside title, creators, publication, description and exact normalized ISBN/ISSN search.
...
- Review presents exactly five groups: **Suggested updates**, **Needs details**, **File problems**, **Cover problems**, and **Imported changes**.
...
- Database-backed catalogue search/filter controls for title/author text, arbitrary substrings, description, folder path, exact creator field, publication/series/periodical title, publication year, publication type, file format, scan status, starred state, exact Nextcloud tag, review flags and root-derived shelf, plus active filter chips, first-run/filtered-empty recovery guidance, desktop and mobile filter controls, loading feedback, lazy high-cardinality typeaheads, and keyboard shortcuts for focus (`/`) and clear (`Escape`).
...
3. **P2 — last opened.** Landed for last-opened activity: Library **Open** clicks update `last_opened_at` before redirecting to `/f/{fileId}`; true page-position history stays deferred to reader integrations.
...
6. **P5 — subjects and classifications. Landed.** Multi-value Library-native subjects and classifications are editable on item details, filterable with dedicated catalogue filters, visible inside card **Details**, and carried through corrected-metadata export/import separate from Nextcloud tags.

Feedback: Nextcloud tags are useful as cross-app labels, but stars, Library-owned reading activity, workflow status, and subjects/classifications should become canonical Library-native fields when they affect catalogue UX, filters, export/import or external metadata-tool handoff.
...
Tag result feedback has landed, and one-click suggested tag buttons have landed for the existing detail-page Nextcloud tag workflow: after add attempts, Library reports added, already assigned, empty and not-assignable outcomes while keeping catalogue cards browse-only and tags separate from Library-native subjects/classifications. The picker only offers currently unassigned visible/assignable tags. Batch tagging now applies or removes a Nextcloud tag from selected visible items up to the safety cap; batch tag removal reports requested/removed/not-tagged/skipped counts. Batch metadata reset has landed for selected visible scanner-conflict items. Batch cover refresh has landed as a read-only no-store retry for selected visible cover URLs. Richer batch metadata changes, app-owned batch cover cache/rebuild and bulk taxonomy workflows remain future work.
...
10. **Metadata portability.** Metadata export foundation.** First slice landed: user-edited catalogue rows can be downloaded as side-effect-free JSON with stable file identity, root/shelf labels, publication metadata and provenance; preview/apply can write matched rows back into an existing Library catalogue; sidecar manifest and ZIP exports provide external-tool handoff paths. Source-folder OPF/JSON writing and fresh-install reconstruction remain outside the app boundary.
11. **Catalogue scale follow-ups.** Count-only badges, projected catalogue DTOs, workload-led indexes, review-flag indexes, facet/typeahead indexes, materialized arbitrary-substring search grams and the conservative unchanged-file metadata fast path have landed. Ordinary trusted indexed files with existing items/current pipeline revision can skip extraction and item writes when the primary plus selected OPF fingerprint is unchanged; weak signals and all changed/error/repair cases extract. Historical alpha.153 exact-package smoke and alpha.157 exact-package smoke measured write elision on a privacy-safe 40-file root and reported zero browser console errors with `release_package_smoke_ok=true`. This does not instrument source writes or compare content bytes, and it is not throughput or latency evidence. Pipeline revision must be bumped for every output-affecting extractor, normalization, sidecar precedence, filename/folder interpretation or ItemService candidate-mapping change. Fresh database migration rehearsal, representative real-library scale gates and SQL-native scanner-conflict counting remain pending; explicit selected-item arrays remain the batch mutation boundary; no elapsed-speedup claim is made.

The import-health review/probe cut remains landed background for the release rehearsal: metadata-error review, review queue shortcuts, tag metadata-error rows, tag scanner-conflict rows, archive/container check, archive magic, cover health, cover support matrix, non-ZIP CBZ guidance, read-only archive extraction, no source files are changed, PDF/EPUB/OPF/CBZ fixture coverage and middle-initial filename authors are documented as current review/diagnostic evidence.
...
1. Keep the exact `0.1.0-alpha.174` package evidence current after any further patch, then repeat the alpha checklist on real mixed files rather than only synthetic/fresh-install fixtures.
