...
11. **Catalogue scale follow-ups.** Count-only smart-view and saved-collection requests no longer fetch list rows/facets for normal filters. Ordinary catalogue/AJAX item DTOs now omit cover override blobs, raw provenance maps, comments and detail-only mutation URLs, and normal catalogue SQL avoids selecting cover override data; tags, descriptions, diagnostics and visible card actions remain. Scanner-conflict and weak-metadata review views retain rich provenance, and detail/cover/export/import remain full. Seven additive user-scoped indexes now support measured catalogue sort/filter and file diagnostic queries without duplicating existing scan-job/root/saved-collection indexes or adding a starred index; redundant legacy single-user indexes remain. `LOWER(...)`, leading-wildcard, JSON and scanner-conflict queries remain outside ordinary B-tree benefits. Duplicated creator landing URLs, description/lazy detail loading, unchanged-rescan short-circuiting, performance instrumentation, the saved raw-tag bug and SQL-native scanner-conflict counting remain pending.
