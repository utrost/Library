...
5. **P4 — Cover quality path.** Preview/CBZ/placeholder covers are good enough for browsing, first EPUB cover extraction from package manifests has landed, and the cover refresh affordance can retry the cover route with no-store response headers while carrying the cover-source explanation as help text. Missing pieces are app-owned cover cache and manual cover override.
...
Tag result feedback has landed, and one-click suggested tag buttons have landed for the existing detail-page Nextcloud tag workflow: after add attempts, Library reports added, already assigned, empty and not-assignable outcomes while keeping catalogue cards browse-only and tags separate from Library-native genres/classifications. The picker only offers currently unassigned visible/assignable tags. A first filter-result batch tagging workflow now applies a Nextcloud tag to all current filter results up to the safety cap. Batch metadata reset has landed for current scanner-conflict results selected by catalogue filters. Richer batch metadata changes, batch cover refresh/rebuild and bulk taxonomy workflows remain future work.
