...
- Database-backed catalogue search/filter controls for title/author text, exact creator field, publication/series/periodical title, publication year, publication type, file format, scan status, starred state, exact Nextcloud tag and root-derived shelf, plus active filter chips and a compact always-visible quick-filter row for search/sort/star/page-size changes.
