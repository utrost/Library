...
4. **P3 — Scan lifecycle repair controls.** Queued per-root scans and progress/history exist, metadata-error retry works from Library settings, and missing-file recheck works from the same repair surface. Missing pieces are cancellation, scheduled/resumable scans and completion/failure notifications.
5. **P4 — Cover quality path.** Preview/CBZ/placeholder covers are good enough for browsing, first EPUB cover extraction from package manifests has landed, and a cover refresh affordance can retry the cover route with no-store response headers. Missing pieces are app-owned cover cache, manual cover override and user-friendly preview-failure explanations.
