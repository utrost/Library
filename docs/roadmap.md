...
3. **P2 — Discovery by publication structure.** Series and periodicals UX is the next common-use priority. Search/filter/pagination are implemented, and the catalogue now supports filtering and sorting by series or periodical title through the existing publication field. A top series and periodicals panel now lists item counts and links into the filtered catalogue, with empty-state guidance when no series metadata exists. The next discovery layer is creator, series, publication and year pages plus saved views or smart collections.
...
6. **P5 — Metadata portability.** Corrected metadata export exists, and **Metadata import preview.** First slice landed: a read-only preview endpoint can parse a Library corrected metadata JSON export and report matched/missing items plus differing fields. More import/export work is lower priority than common catalogue UX; for now, apply imported metadata remains future work, as does OPF/JSON sidecar write-back so corrections become File-First durable.
