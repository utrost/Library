...
11. **Catalogue scale follow-ups.** Count-only badges, projected catalogue DTOs, workload-led indexes and the conservative unchanged-file metadata fast path have landed. Ordinary trusted indexed files with existing items/current pipeline revision can skip extraction and item writes when the primary plus selected OPF fingerprint is unchanged; weak signals and all changed/error/repair cases extract. Exact-package smoke measured write elision on a privacy-safe 40-file root: warm-up rewrote 40 item rows and established 40 markers, while the unchanged second scan rewrote zero item rows, retained all 40 markers and reported `source_observation_changes=0` for path/ETag/mtime/size/MIME observations; both scans indexed all 40 files without missing files or errors. This does not instrument source writes or compare content bytes, and it is not throughput or latency evidence. Pipeline revision must be bumped for every output-affecting extractor, normalization, sidecar precedence, filename/folder interpretation or ItemService candidate-mapping change. Performance instrumentation, duplicated creator URL cleanup, lazy descriptions, the saved raw-tag fix and SQL-native scanner-conflict counting remain pending; no elapsed-speedup claim is made.
