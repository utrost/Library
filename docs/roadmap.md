...
11. **Catalogue scale follow-ups.** Count-only smart-view and saved-collection requests no longer fetch list rows/facets for normal filters. List-payload trimming, workload-led catalogue indexes, unchanged-rescan short-circuiting, runtime PHP database tests (including saved raw-tag count correctness), and SQL-native scanner-conflict counting remain pending.
