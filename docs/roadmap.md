...
3. **P2 — Discovery by publication structure.** Series and periodicals UX is the next common-use priority. Search/filter/pagination are implemented, and the catalogue now supports filtering and sorting by series or periodical title through the existing publication field. A top series and periodicals panel now lists item counts and links into the filtered catalogue. The next discovery layer is creator, series, publication and year pages plus saved views or smart collections.
