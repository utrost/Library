...
7. **P6 — Root/onboarding/shared-library polish.** Root lifecycle works, and first-run/empty-state guidance now explains the add-root → scan → browse path. Remaining release polish is stronger destructive-action confirmation, recovery copy and eventually admin-managed shared-root provisioning.
