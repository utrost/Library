...
2. **P1 — Metadata correction workflow.** Details editing works, but one user edit protects the whole item. The P1 field-level provenance foundation is now started: scanner/user field sources and current field values are stored as JSON maps and displayed read-only on the details page while preserving item-level `user_edited` protection. The single-field reset-to-scanner path is now starting so individual fields can be restored from stored scanner candidates without dropping the whole user-edited item. Next slices: whole-item reset/reconciliation rules, date/language/creator validation guidance and eventually a review queue for scanner/sidecar/user conflicts.
