...
7. **P6 — Root/onboarding/shared-library polish.** Root lifecycle works, typed root-delete confirmation is in place for destructive app-row cleanup, and first-run/empty-state guidance now explains the add-root → scan → browse path. Remaining release polish is recovery copy, richer validation and eventually admin-managed shared-root provisioning.
...
4. **Root removal policy.** First slice landed: root deletion uses the conservative v0.1 behaviour, removing app-owned root/index/catalogue rows only, asks users to type `DELETE`, and avoids deleting source files from Library. Remaining polish is clearer recovery copy before deletion.
