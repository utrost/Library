...
1. The app installs/enables from `dist/library-0.1.0-alpha.123.tar.gz` on a Nextcloud 34 test instance.
...
Setup: install `dist/library-0.1.0-alpha.123.tar.gz` into a disposable Nextcloud 34 instance and enable Library.
...
Expected result: both pages load; the app is version `0.1.0-alpha.123`; no Library-specific JavaScript error appears.
