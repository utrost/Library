...
1. The app installs/enables from `dist/library-0.1.0-alpha.163.tar.gz` on a Nextcloud 34 test instance.
...
Setup: install `dist/library-0.1.0-alpha.163.tar.gz` into a disposable Nextcloud 34 instance and enable Library.
...
Expected result: both pages load; the app is version `0.1.0-alpha.163`; no Library-specific JavaScript error appears. Library shows the catalogue, Review shows the focused native cleanup destination (including a clear empty state when appropriate), browser back/forward and copied queue URLs preserve the selected filter, and Settings reaches the existing personal-settings page.
