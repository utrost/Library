...
1. The app installs/enables from `dist/library-0.1.0-alpha.160.tar.gz` on a Nextcloud 34 test instance.
...
Setup: install `dist/library-0.1.0-alpha.160.tar.gz` into a disposable Nextcloud 34 instance and enable Library.
...
Expected result: both pages load; the app is version `0.1.0-alpha.160`; no Library-specific JavaScript error appears. The catalogue page shows the native Library/Review/Settings shell scaffold, Review opens scanner-conflict results, and Settings reaches the existing personal-settings page.
