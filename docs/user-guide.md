...
2. Improve the metadata correction workflow. Details editing exists, and field-level provenance is recorded and shown read-only on item details, but reset-to-scanner is still future work; conflict review and validation guidance are also still missing.
