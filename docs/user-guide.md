...
3. **Metadata correction workflow** — details editing, field-level scanner candidates, single-field reset-to-scanner, whole-item reset to scanner candidates, non-blocking edit guidance and field-level **Differs from scanner** labels exist, but there is no bulk edit, review queue or hard validation.
