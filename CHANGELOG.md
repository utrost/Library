...
- Added grouped catalogue filter reset actions, Home/Shelves active-filter explanations, pending draft indicators for delayed text/typeahead inputs, and filtered-empty reset suggestions that point at the matching active chip.
- Delayed server-backed publication/series, creator, publisher, and subject typeahead requests until three trimmed characters are entered, while retaining the two-character year threshold and immediate exact-value application for selected suggestions.
