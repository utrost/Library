...
try {
  ...
  if (!token) {
    ...
    console.log(`source_has_useful_views_strip=${sourceComponent.includes('library-useful-views') && sourceComponent.includes('smartViews') && sourceComponent.includes('Useful views') && sourceComponent.includes('sort=lastOpened') && sourceComponent.includes('workflowStatus=to-read') && sourceComponent.includes('needsMetadata=1') && sourceComponent.includes('scannerConflicts=1') && sourceComponent.includes('status=metadata_error') && sourceComponent.includes('coverReview=placeholder') && sourceComponent.includes('noCreator=1') && sourceComponent.includes('noPublication=1') && sourceComponent.includes('weakMetadata=filename') && sourceComponent.includes('unreviewedImports=1') && sourceComponent.includes('library-useful-view-count') && sourceComponent.includes('Empty useful views mean no current catalogue items match that saved direction yet')}`)
    ...
    console.log(`app_version=0.1.0-alpha.133`)
    ...
    console.log(`source_has_compact_cover_cards_all_widths=${appInfo.includes('<version>0.1.0-alpha.133</version>') && sourceComponent.includes('library-cover-details') && sourceComponent.includes('Show details and actions') && sourceStyle.split('@media (max-width: 520px)', 1)[0].includes('grid-template-columns: repeat(auto-fill, minmax(120px, 1fr))') && sourceStyle.split('@media (max-width: 520px)', 1)[0].includes('min-height: 0')}`)
    ...
    console.log(`settings_has_scan_changes_panel=${sourceSettingsTemplate.includes('library-scan-changes-panel') && sourceSettingsTemplate.includes('Changes found') && sourceSettingsTemplate.includes('filesAdded') && sourceSettingsTemplate.includes('pathsUpdated')}`)
