...
try {
  ...
  if (!token) {
    ...
    const coverRefreshMatch = detail.text.match(/href="([^"]*\/apps\/library\/items\/[^\"]*\/cover\?refresh=1[^"]*)"[^>]*library-cover-refresh-action/)
    const coverRefreshPath = coverRefreshMatch ? coverRefreshMatch[1].replaceAll('&amp;', '&') : ''
    ...
    console.log(`app_version=0.1.0-alpha.78`)
    ...
    console.log(`source_has_compact_cover_cards_all_widths=${appInfo.includes('<version>0.1.0-alpha.78</version>') && sourceComponent.includes('library-cover-details') && sourceComponent.includes('Show details and actions') && sourceStyle.split('@media (max-width: 520px)', 1)[0].includes('grid-template-columns: repeat(auto-fill, minmax(120px, 1fr))') && sourceStyle.split('@media (max-width: 520px)', 1)[0].includes('min-height: 0')}`)
    ...
    console.log(`detail_has_cover_refresh_action=${detail.text.includes('library-cover-refresh-action') && detail.text.includes('Refresh cover preview') && detail.text.includes('refresh=1')}`)
    console.log(`detail_has_cover_quality_explanation=${detail.text.includes('library-cover-quality-explanation') && detail.text.includes('How Library chose this cover') && detail.text.includes('If you see a placeholder')}`)
    ...
    console.log(`settings_has_retry_metadata_errors_form=${settingsPage.text.includes('library-scan-retry-metadata-errors-form') && settingsPage.text.includes('Retry metadata errors') && settingsPage.text.includes('metadata_error are retried')}`)
    console.log(`settings_has_recheck_missing_files_form=${settingsPage.text.includes('library-scan-recheck-missing-files-form') && settingsPage.text.includes('Recheck missing files') && settingsPage.text.includes('missing files are rechecked')}`)
