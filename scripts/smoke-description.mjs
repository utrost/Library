...
const tokenName = `hermes-library-description-smoke-${Date.now()}`
const uniqueDescription = `Hermes description search ${Date.now()}`
...
try {
  ...
  if (!token) {
    ...
    if (!itemId || !first.detailsUrl || !state?.metadataExportUrl) {
      fail('catalogue_item_missing_description_prerequisites')
    } else {
      originalState = itemState(itemId)
      setDescription(itemId, uniqueDescription)
      const searchPage = await fetchText(`/apps/library/?q=${encodeURIComponent(uniqueDescription)}&limit=1`, token)
      const searchState = decodeInitialState(searchPage.text)
      const searchFirst = searchState?.items?.[0] || {}
      ...
      console.log(`description_item_id=${itemId}`)
      console.log(`description_search_http=${searchPage.status}`)
      console.log(`description_search_first_matches=${Number(searchFirst.id || 0) === itemId}`)
      console.log(`description_search_result_contains_description=${searchFirst.description === uniqueDescription}`)
      console.log(`description_detail_http=${detail.status}`)
      console.log(`description_detail_has_textarea=${detail.text.includes('name="description"')}`)
      console.log(`description_detail_contains_description=${detail.text.includes(uniqueDescription)}`)
      console.log(`description_export_http=${exported.status}`)
      console.log(`description_export_contains_description=${exportedItem?.description === uniqueDescription}`)

      if (searchPage.status !== 200 || Number(searchFirst.id || 0) !== itemId || searchFirst.description !== uniqueDescription || detail.status !== 200 || !detail.text.includes('name="description"') || !detail.text.includes(uniqueDescription) || exported.status !== 200 || exportedItem?.description !== uniqueDescription) {
        fail('description_contract_failed')
      } else {
        console.log('description_search_smoke_ok=true')
      }
    }
  }
} catch (error) {
  ...
  if (itemId && originalState) {
    ...
    console.log(`description_restored=${itemState(itemId).description === (originalState.description || null)}`)
  }
  if (token) {
    try {
