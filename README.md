...
- show indexed files and publication items with a catalogue-first Vue/Vite-backed catalogue page mounted in a conventional Nextcloud `#app-content` shell, responsive, paginated cover gallery, preview-backed covers, placeholder fallbacks, browse-only catalogue cards and built-in useful views for frequent catalogue destinations;
- search and filter the catalogue by title/author/description text, filename and folder path search, exact creator field, publication/series/periodical title, publication year, publication type, file format, scan status, scanner conflicts, exact Nextcloud tag and root-derived shelf;
...
Current v0.1 testing candidate: `0.1.0-alpha.130`.
