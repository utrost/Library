...
- open dedicated publication discovery pages from top series/periodical shortcuts with **Publication contents** read-only issue/date grouping, stable issue/date order, year/month and volume buckets, conservative gap hints, and an **Unknown issue/date** bucket that keeps weak comics and periodicals visible; open a dedicated publication year discovery page from the years shortcut panel, and a dedicated creator discovery page from the creator shortcut panel, keeping compact catalogue cards inside a named discovery context;
