...
- open dedicated publication discovery pages from top series/periodical shortcuts with **Publication contents** read-only issue/date grouping, a horizontal visual issue strip, stable issue/date order, year/month and volume buckets, conservative gap hints, and an **Unknown issue/date** bucket that keeps weak comics and periodicals visible; open a dedicated publication year discovery page from the years shortcut panel, and a dedicated creator discovery page from the creator shortcut panel, keeping compact catalogue cards inside a named discovery context;
- catalogue cards are browse-only, compact and cover-first on mobile and desktop: the default card surface is cover, title, **Read** and **Details**, while secondary metadata/actions stay behind the Details disclosure; a Compact / Gallery / Shelf view-mode toggle lets users switch from the fast default to larger gallery covers or a horizontal shelf view; cover loading uses a subtle shimmer and broken-cover fallback; a lightweight details drawer lets users peek from the catalogue, while the details page owns publication metadata, tag and comment editing, and the publication metadata panel now uses one editable field surface instead of repeating the same values in separate read-only and edit sections;
...
- extract first local metadata candidates from EPUB package OPF, standalone OPF files and basic PDF info dictionaries, including UTF-16 BOM, PDF hex Info strings, literal octal escapes, nested PDF literal parentheses, non-BOM single-byte PDF Info strings, PDF Subject-as-subtitle, normalized PDF CreationDate/ModDate values, plain PDF Info dates and PDF literal line continuations from real PDFs;
...
Current v0.1 testing candidate: `0.1.0-alpha.140`.
