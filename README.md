...
- extract first local metadata candidates from EPUB package OPF, standalone OPF files and basic PDF info dictionaries, including UTF-16 BOM, PDF hex Info strings, literal octal escapes, non-BOM single-byte PDF Info strings, PDF Subject-as-subtitle and normalized PDF CreationDate/ModDate values from real PDFs;
