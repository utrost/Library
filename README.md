...
Current source candidate: `0.1.0-alpha.162`. Catalogue and Review cards now open an accessible native item-detail sidebar with shareable, back/forward-safe URLs, read-only metadata/provenance or review context, and explicit loading/error/unavailable states. Read and the PHP full-details page remain explicit actions; Settings remains the existing personal-settings route.
