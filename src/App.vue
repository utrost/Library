...
function activeGroupChips(group) {
  ...
  return resettableFilterChips.value.filter((chip) => keys.has(chip.key))
}
const activeFilterGroups = computed(() => filterGroups.map((group) => ({ ...group, chips: activeGroupChips(group) })))
const activeFilterCalloutUrl = computed(() => {
  const params = new URLSearchParams()
  for (const chip of resettableFilterChips.value) params.set(chip.key, chip.value)
  const query = params.toString()
  return `${catalogueRootUrl.value}${query ? `?${query}` : ''}`
})
const firstResettableFilterChip = computed(() => resettableFilterChips.value[0] || null)
const hasPendingQuickSearch = computed(() => quickSearch.value.trim() !== String(activeFilters.q || '').trim())
const hasClearableSearch = computed(() => String(activeFilters.q || '').trim() !== '' || hasPendingQuickSearch.value)
function pendingDraftValue(key) {
  const draftMap = {
    q: quickSearch,
  ...
  return (draftMap[key]?.value ?? '').trim() !== String(activeFilters[key] || '').trim()
}
function pendingDraftText(key) {
  if (!pendingDraftValue(key)) return ''
  if (key === 'q') return t('library', 'Not applied yet — press Enter or Apply.')
  return t('library', 'Press Enter or Apply to use this value.')
}
function pendingApplyClass(key) {
  return { 'library-filter-apply--pending': pendingDraftValue(key) }
}
...
function clearFilterGroupParams(groupKey) {
  const group = filterGroups.find((candidate) => candidate.key === groupKey)
  const groupKeys = new Set(group?.keys || [])
  const params = new URLSearchParams()
  for (const [param, value] of Object.entries(activeFilters)) {
    const normalized = String(value || '').trim()
    if (normalized !== '' && !groupKeys.has(param) && !(param === 'sort' && normalized === 'title') && !(param === 'view' && normalized === 'compact')) {
      params.set(param, normalized)
    }
  }
  params.delete('page')
...
function clearFilterGroupUrl(groupKey) {
  const params = clearFilterGroupParams(groupKey)
  const query = params.toString()
  return `${catalogueRootUrl.value}${query ? `?${query}` : ''}`
}
...
function clearFilterGroup(groupKey) {
  ...
  if (!group) return
  const params = clearFilterGroupParams(groupKey)
  for (const key of group.keys) activeFilters[key] = activeFilterDefaults[key]
  void submitFiltersAjax(null, {
    params,
    generation: ++catalogueRequestGeneration,
  })
...
<template>
      ...
      <template #footer>
        <section class="library-sidebar-filter-section" aria-labelledby="library-sidebar-filters-heading">
          ...
          <form method="get" class="library-filter-bar library-sidebar-filters" :aria-label="t('library', 'Catalogue search and filters')" @submit.prevent="applyAllSearchFilters">
            ...
            </fieldset><fieldset class="library-filter-group" data-library-filter-group="location"><legend>{{ t('library', 'Location') }}</legend><a v-if="activeFilterGroups.find((group) => group.key === 'location')?.chips.length" :href="clearFilterGroupUrl('location')" class="button tertiary library-filter-group-clear" @click.prevent="clearFilterGroup('location')">{{ t('library', 'Clear Location') }}</a><label>{{ t('library', 'Shelf') }}<select v-model="activeFilters.shelf" name="shelf" @change="submitFiltersNow($event)"><option value="">{{ t('library', 'All shelves') }}</option><option v-for="shelf in shelves" :key="shelf" :value="shelf">{{ shelf }}</option></select></label>
            <div class="library-folder-filter"><label for="library-folder-search">{{ t('library', 'Folder') }}</label><input id="library-folder-search" v-model="folderSearch" type="search" name="folderSearch" autocomplete="off" :placeholder="t('library', 'Type at least 3 path characters')" :title="t('library', 'Select an exact folder path')" role="combobox" aria-autocomplete="list" aria-controls="library-folder-suggestions" :aria-expanded="folderSearchFocused && folderSuggestions.length > 0 ? 'true' : 'false'" @focus="folderSearchFocused = true" @keydown.escape="folderSearchFocused = false"><ul v-if="folderSearchFocused && folderSuggestions.length > 0" id="library-folder-suggestions" class="library-folder-suggestions" role="listbox"><li v-for="folder in folderSuggestions" :key="folder" role="option"><button type="button" class="library-folder-suggestion" @mousedown.prevent @click="selectFolderSuggestion(folder, $event)">{{ folder }}</button></li></ul><button type="submit" class="button secondary library-folder-apply" :class="pendingApplyClass('folder')">{{ t('library', 'Apply folder') }}</button></div>
            </fieldset><fieldset class="library-filter-group" data-library-filter-group="review"><legend>{{ t('library', 'Review') }}</legend><a v-if="activeFilterGroups.find((group) => group.key === 'review')?.chips.length" :href="clearFilterGroupUrl('review')" class="button tertiary library-filter-group-clear" @click.prevent="clearFilterGroup('review')">{{ t('library', 'Clear Review') }}</a><label>{{ t('library', 'Scan status') }}<select v-model="activeFilters.status" name="status" @change="submitFiltersNow($event)"><option value="">{{ t('library', 'All scan statuses') }}</option><option v-for="status in scanStatuses" :key="status" :value="status">{{ status }}</option></select></label>
            ...
            </fieldset><fieldset class="library-filter-group" data-library-filter-group="personal"><legend>{{ t('library', 'Personal / display') }}</legend><a v-if="activeFilterGroups.find((group) => group.key === 'personal')?.chips.length" :href="clearFilterGroupUrl('personal')" class="button tertiary library-filter-group-clear" @click.prevent="clearFilterGroup('personal')">{{ t('library', 'Clear Personal / display') }}</a></fieldset><button type="submit" class="button primary">{{ t('library', 'Apply filters') }}</button><a v-if="resettableFilterChips.length > 0" :href="clearAllFiltersUrl()" class="button secondary" @click.prevent="clearAllFilters">{{ t('library', 'Clear') }}</a>
          </form>
        </section>
  ...
  <main v-else-if="isHome" id="library-home" class="library-panel library-home" aria-labelledby="library-home-heading">
    ...
    <aside v-if="resettableFilterChips.length > 0" class="library-active-filter-callout" :aria-label="t('library', 'Active catalogue filters')">
      <h3>{{ t('library', 'Active catalogue filters') }}</h3>
      <nav class="library-active-filter-callout-chips" :aria-label="t('library', 'Active catalogue filters')">
        <a v-for="chip in resettableFilterChips" :key="`callout-${chip.key}`" :href="filterChipRemoveUrl(chip.key)" class="library-filter-chip" :aria-label="`${t('library', 'Remove filter')}: ${chip.label}`" @click.prevent="removeFilterChip(chip.key)"><strong>{{ chip.label }}<template v-if="chip.displayValue">:</template></strong><template v-if="chip.displayValue"> {{ ' ' }}<span class="library-filter-chip-value" :title="chip.value">{{ chip.displayValue }}</span></template> <span aria-hidden="true">×</span></a>
      </nav>
  ...
  <section v-else id="library-catalogue" class="library-panel library-mobile-compact-chrome" :class="{ 'library-catalogue--loading': catalogueRequestState.loading }" aria-labelledby="library-catalogue-heading" :aria-busy="catalogueRequestState.loading ? 'true' : 'false'">
    ...
    <div v-if="items.length === 0" class="library-empty-content" :class="{ 'library-first-run-guidance': hasNoConfiguredRoots || hasNoEnabledRoots, 'library-filter-empty-state': hasActiveFilters && !hasNoConfiguredRoots && !hasNoEnabledRoots }" role="status">
      ...
      <template v-else-if="hasActiveFilters">
        <h3 :title="t('library', 'Try a broader search, remove one active chip, or clear every catalogue filter.')">{{ t('library', 'No items match these filters') }}</h3>
        <nav v-if="resettableFilterChips.length > 0" class="library-empty-filter-chips" :aria-label="t('library', 'Remove active filters')"><a v-for="chip in resettableFilterChips" :key="`empty-${chip.key}`" :href="filterChipRemoveUrl(chip.key)" class="library-filter-chip" :aria-label="`${t('library', 'Remove filter')}: ${chip.label}`" @click.prevent="removeFilterChip(chip.key)"><strong>{{ chip.label }}<template v-if="chip.displayValue">:</template></strong><template v-if="chip.displayValue"> {{ ' ' }}<span class="library-filter-chip-value" :title="chip.value">{{ chip.displayValue }}</span></template> <span aria-hidden="true">×</span></a></nav>
        <p v-if="firstResettableFilterChip" class="library-muted">{{ t('library', 'Try removing {filter}.', { filter: firstResettableFilterChip.displayValue ? `${firstResettableFilterChip.label}: ${firstResettableFilterChip.displayValue}` : firstResettableFilterChip.label }) }}</p>
        <p class="library-empty-actions"><a v-if="hasClearableSearch" :href="clearSearchUrl()" class="button secondary library-empty-clear-search" @click.prevent="removeFilterChip('q')">{{ t('library', 'Clear search') }}</a><a v-if="resettableFilterChips.length > 0" :href="clearAllFiltersUrl()" class="button primary" @click.prevent="clearAllFilters">{{ t('library', 'Clear all filters') }}</a></p>
      </template>
      <template v-else>
