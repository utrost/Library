...
def test_page_controller_accepts_publication_filter_and_provides_publication_facets():
    ...
    assert "facets' => ['shelves' => [], 'formats' => [], 'scanStatuses' => ['indexed', 'metadata_error', 'missing'], 'workflowStatuses' => [], 'publications' => [], 'publicationSummaries' => [], 'publicationYears' => [], 'creators' => []]" in page
...
def test_docs_pivot_priority_from_import_export_to_series_periodical_ux():
    ...
    assert "The next discovery layer is creator, series, publication and year pages plus saved views or smart collections." in roadmap
