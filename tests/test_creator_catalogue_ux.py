...
def test_page_controller_accepts_creator_filter_and_provides_creator_facets():
    ...
    assert "foreach (['q', 'type', 'publication', 'year', 'creator', 'format', 'tag', 'shelf', 'status', 'sort'] as $param)" in page
    assert "@param array{q:string,type:string,publication:string,year:string,creator:string,tag:string,shelf:string,format:string,status:string,sort:string}" in page
