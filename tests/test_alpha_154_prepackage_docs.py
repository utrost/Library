...
def test_alpha_154_is_consistently_a_source_candidate_with_package_live_evidence_pending():
    ...
    assert "Current source candidate: `0.1.0-alpha.154`." in readme
    assert "Exact alpha.154 package, checksum, install, migration, 40-file scan, API, and browser evidence is pending." in readme
    assert "performance instrumentation is still pending" not in readme
    assert "exact-package alpha.154 rehearsal" in readiness and "remain pending" in readiness
    assert "alpha.154 exact-package and live migration rehearsal remain pending" in risk
    assert "No alpha.154 archive, checksum verification, install, database migration, 40-file scan, API smoke, or browser smoke has been completed" in release
...
def test_all_40_file_and_package_live_evidence_is_explicitly_historical_alpha_153():
    ...
    assert "Release baseline: the exact alpha.154 package passed" not in checklist
