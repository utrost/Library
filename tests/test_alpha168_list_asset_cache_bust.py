...
def test_main_template_and_vite_define_report_the_alpha170_app_version():
    assert 'data-library-version="0.1.0-alpha.170"' in read("templates/main.php")
    assert "appVersion: JSON.stringify('0.1.0-alpha.170')" in read("vite.config.js")
...
def test_list_view_asset_path_is_cache_busted_after_alpha170_deploy():
    ...
    assert script == "library-main-0-1-0-alpha-170-pathlink"
    assert style == "library-vue-0-1-0-alpha-170-pathlink"
...
def test_release_packaging_uses_page_controller_wired_assets(tmp_path):
    ...
    manifest = subprocess.run(
        [
            ...
            "console.log(JSON.stringify(releaseFrontendFiles('0.1.0-alpha.170')))",
        ],
        cwd=ROOT,
        check=True,
        capture_output=True,
    ...
    subprocess.run(
        ["node", "scripts/stage-release-frontend.mjs", str(stage), "0.1.0-alpha.170"],
        cwd=ROOT,
        check=True,
        capture_output=True,
        text=True,
