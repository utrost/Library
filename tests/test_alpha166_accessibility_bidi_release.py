...
def test_alpha166_version_sources_and_unpackage_boundary():
    ...
    assert "library-main-0-1-0-alpha-173-cover-help-tooltip" in read("lib/Controller/PageController.php")
    assert "library-vue-0-1-0-alpha-173-cover-help-tooltip" in read("lib/Controller/PageController.php")
...
def test_docs_are_truthful_candidate_with_manual_at_debt():
    ...
    assert "0.1.0-alpha.173" in release
    ...
    for path in ["README.md", "CHANGELOG.md", "docs/roadmap.md", "docs/current-state-and-risk-register.md", "docs/app-store-readiness.md", "docs/architecture-review.md", "docs/alpha-test-checklist.md"]:
        assert "alpha.173" in read(path), path
    assert "alpha.166" in read("CHANGELOG.md")
...
def test_release_operative_artifact_instructions_name_alpha166_only():
    ...
    required = [
        ...
        "sha256sum -c library-0.1.0-alpha.173.tar.gz.sha256",
        "tar -xzf dist/library-0.1.0-alpha.173.tar.gz",
        "openssl dgst -sha512 -sign ~/.nextcloud/certificates/library.key dist/library-0.1.0-alpha.173.tar.gz",
    ]
...
def test_second_review_field_level_isolation_and_current_identity():
    ...
    assert readiness.count("Current candidate baseline: `0.1.0-alpha.173`") == 1
    release = read("RELEASE.md")
