...
def test_vue_catalogue_uses_nextcloud_l10n_for_visible_copy():
    ...
    assert "import { t } from '@nextcloud/l10n'" in main
    assert "t('library', 'Publication catalogue')" in app
    assert "t('library', 'Publication catalogue')" in main
    assert "t('library', 'Apply filters')" in app
    assert "t('library', 'Apply filters')" in main
...
def test_catalogue_sections_and_controls_have_stable_accessible_names():
    ...
    assert "panel.setAttribute('aria-labelledby', 'library-catalogue-heading')" in main
    assert "heading.id = 'library-catalogue-heading'" in main
