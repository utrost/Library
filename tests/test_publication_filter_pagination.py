...
def test_page_controller_pagination_preserves_publication_filter():
    ...
    assert "foreach (['q', 'type', 'publication', 'year', 'creator', 'format', 'tag', 'shelf', 'status', 'starred', 'sort'] as $param)" in controller
    assert "@param array{q:string,type:string,publication:string,year:string,creator:string,tag:string,shelf:string,format:string,status:string,starred:string,sort:string}" in controller
