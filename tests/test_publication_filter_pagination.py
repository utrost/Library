...
def test_page_controller_pagination_preserves_publication_filter():
    ...
    assert "foreach (['q', 'type', 'publication', 'year', 'format', 'tag', 'shelf', 'status', 'sort'] as $param)" in controller
    assert "@param array{q:string,type:string,publication:string,year:string,tag:string,shelf:string,format:string,status:string,sort:string}" in controller
