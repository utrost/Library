...
def test_alpha_157_version_assets_and_release_contracts_are_aligned():
    assert "<version>0.1.0-alpha.169</version>" in read("appinfo/info.xml")
    assert '"version": "0.1.0-alpha.169"' in read("package.json")
    assert '"version": "0.1.0-alpha.169"' in read("package-lock.json")
    ...
    assert "v0.1.0-alpha.169" in read("CHANGELOG.md")
...
def test_app_store_readiness_distinguishes_current_source_from_alpha_159_evidence():
    ...
    assert "Current candidate baseline: `0.1.0-alpha.169`" in readiness
