...
def test_batch_apply_distinguishes_absent_item_ids_from_supplied_values():
    ...
    assert "$this->itemService->itemIdsForCatalogueFilters($user->getUID(), $filters, 5000)" not in body
    assert "applyBatchMetadataEdit" in body
...
def test_controller_contains_closed_canonical_explicit_id_parser():
    ...
    assert "public static function parse(mixed $value): array" in selected
    parser = selected.split("public static function parse", 1)[1]
    assert "!is_array($value) || $value === []" in parser
    assert "InvalidArgumentException" in parser
