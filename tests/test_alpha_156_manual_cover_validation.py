...
def test_alpha_156_release_assets_and_runtime_suite_are_wired():
    assert "<version>0.1.0-alpha.156</version>" in read("appinfo/info.xml")
    assert '"version": "0.1.0-alpha.156"' in read("package.json")
    assert '"version": "0.1.0-alpha.156"' in read("package-lock.json")
    ...
    assert "v0.1.0-alpha.156" in read("CHANGELOG.md")
