...
def test_vue_component_css_is_built_to_nextcloud_css_asset_and_loaded():
    ...
    assert "private const VUE_SCRIPT_ASSET = 'library-main-0-1-0-alpha-155';" in controller
    assert "private const VUE_STYLE_ASSET = 'library-vue-0-1-0-alpha-155';" in controller
    ...
    assert (ROOT / "js" / "library-main-0-1-0-alpha-155.mjs").exists()
    assert (ROOT / "css" / "library-vue-0-1-0-alpha-155.css").exists()
