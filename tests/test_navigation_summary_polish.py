...
def test_catalogue_uses_single_result_summary_and_compact_pagination_label():
    ...
    assert "library-pagination-range" in fallback
    assert "{{ t('library', 'Page') }} {{ pagination.page }}" in app
    assert "Page ${pagination.page" in fallback
...
def test_navigation_summary_polish_version_bump_is_tracked_for_asset_refresh():
    ...
    assert "<version>0.1.0-alpha.111</version>" in info
    assert '"version": "0.1.0-alpha.111"' in package
    assert '"version": "0.1.0-alpha.111"' in lock
