...
def test_alpha_158_version_assets_and_release_language_are_aligned():
    assert "<version>0.1.0-alpha.171</version>" in read("appinfo/info.xml")
    assert '"version": "0.1.0-alpha.171"' in read("package.json")
    assert '"version": "0.1.0-alpha.171"' in read("package-lock.json")
    page = read("lib/Controller/PageController.php")
    assert "library-main-0-1-0-alpha-171-cover-overlay-4" in page
    assert "library-vue-0-1-0-alpha-171-cover-overlay-4" in page
