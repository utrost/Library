...
This repo is prepared for the v0.1 manual testing pass from app version `0.1.0-alpha.156`. The existing public prerelease tag is `v0.1.0-alpha.1`; create a new tag only after the manual test pass is accepted.

## Release type

- Private/early-tester v0.1 candidate on a Nextcloud 34 test instance.
- App version in `appinfo/info.xml`: `0.1.0-alpha.156`.
...
```text
dist/library-0.1.0-alpha.156.tar.gz
dist/library-0.1.0-alpha.156.tar.gz.sha256
```
...
```bash
openssl dgst -sha512 -sign ~/.nextcloud/certificates/library.key dist/library-0.1.0-alpha.156.tar.gz | openssl base64
```
...
- The checksum for the exact alpha.153 archive passed, and that archive was installed and enabled. This evidence must not be treated as alpha.156 package evidence.
...
The historical 40-file result verifies write elision only. Alpha.154 adds measured aggregate operation boundaries, but those observations are not proof of a universal speedup. No alpha.156 archive, checksum verification, install, database migration, 40-file scan, API smoke, or browser smoke has been completed in this source-only release update.

For a non-Docker disposable instance, the equivalent manual steps are:

```bash
(cd dist && sha256sum -c library-0.1.0-alpha.156.tar.gz.sha256)
rm -rf /var/www/html/custom_apps/library
mkdir -p /var/www/html/custom_apps
tar -xzf dist/library-0.1.0-alpha.156.tar.gz -C /var/www/html/custom_apps
