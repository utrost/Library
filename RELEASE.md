...
This repo is prepared for the v0.1 manual testing pass from app version `0.1.0-alpha.160`. The existing public prerelease tag is `v0.1.0-alpha.1`; create a new tag only after the manual test pass is accepted.

## Release type

- Private/early-tester v0.1 candidate on a Nextcloud 34 test instance.
- App version in `appinfo/info.xml`: `0.1.0-alpha.160`.
...
```text
dist/library-0.1.0-alpha.160.tar.gz
dist/library-0.1.0-alpha.160.tar.gz.sha256
```
...
```bash
openssl dgst -sha512 -sign ~/.nextcloud/certificates/library.key dist/library-0.1.0-alpha.160.tar.gz | openssl base64
```
...
That script verifies and installs the exact generated archive before running live smokes. The generated `.sha256` file is the archive checksum sidecar and is not packaged; `/tmp/library-alpha160-codex-remediation-3.md` is the authoritative package-excluded run evidence, not a sidecar. Prior alpha.160 results predate that latest rereview remediation and are superseded as acceptance evidence. Consult remediation 3 for the authoritative rebuilt artifact identity, timestamp, commands, and outcomes. The following alpha.159 section remains historical evidence.
...
```bash
(cd dist && sha256sum -c library-0.1.0-alpha.160.tar.gz.sha256)
rm -rf /var/www/html/custom_apps/library
mkdir -p /var/www/html/custom_apps
tar -xzf dist/library-0.1.0-alpha.160.tar.gz -C /var/www/html/custom_apps
