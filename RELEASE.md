...
This repo is prepared for Uwe's v0.1 manual testing pass from app version `0.1.0-alpha.130`. The existing public prerelease tag is `v0.1.0-alpha.1`; create a new tag only after Uwe's manual test pass is accepted.

## Release type

- Private/early-tester v0.1 candidate on a Nextcloud 34 test instance.
- App version in `appinfo/info.xml`: `0.1.0-alpha.130`.
...
```text
dist/library-0.1.0-alpha.130.tar.gz
dist/library-0.1.0-alpha.130.tar.gz.sha256
```

The archive contains a top-level `library-0.1.0-alpha.130/` directory and excludes `.git`, `.github`, `node_modules`, `build`, `dist`, caches and bytecode.
...
That script verifies the archive checksum, disables the currently installed app if present, removes the development copy from `custom_apps/library`, extracts `dist/library-0.1.0-alpha.130.tar.gz`, fixes ownership/permissions, runs PHP lint on key app files, enables the app, runs `occ upgrade`, lists app routes and runs the live Vue/browser smoke scripts.

For a non-Docker disposable instance, the equivalent manual steps are:

```bash
sha256sum -c dist/library-0.1.0-alpha.130.tar.gz.sha256
rm -rf /var/www/html/custom_apps/library
mkdir -p /var/www/html/custom_apps
tar -xzf dist/library-0.1.0-alpha.130.tar.gz -C /var/www/html/custom_apps
mv /var/www/html/custom_apps/library-0.1.0-alpha.130 /var/www/html/custom_apps/library
