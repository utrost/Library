...
This repo is prepared for Uwe's v0.1 manual testing pass from app version `0.1.0-alpha.114`. The existing public prerelease tag is `v0.1.0-alpha.1`; create a new tag only after Uwe's manual test pass is accepted.

## Release type

- Private/early-tester v0.1 candidate on a Nextcloud 34 test instance.
- App version in `appinfo/info.xml`: `0.1.0-alpha.114`.
...
```text
dist/library-0.1.0-alpha.114.tar.gz
dist/library-0.1.0-alpha.114.tar.gz.sha256
```

The archive contains a top-level `library-0.1.0-alpha.114/` directory and excludes `.git`, `.github`, `node_modules`, `build`, `dist`, caches and bytecode.
