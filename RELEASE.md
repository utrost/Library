...
This repo is prepared for the v0.1 manual testing pass from app version `0.1.0-alpha.150`. The existing public prerelease tag is `v0.1.0-alpha.1`; create a new tag only after the manual test pass is accepted.

## Release type

- Private/early-tester v0.1 candidate on a Nextcloud 34 test instance.
- App version in `appinfo/info.xml`: `0.1.0-alpha.150`.
...
```text
dist/library-0.1.0-alpha.150.tar.gz
dist/library-0.1.0-alpha.150.tar.gz.sha256
```
...
```bash
openssl dgst -sha512 -sign ~/.nextcloud/certificates/library.key dist/library-0.1.0-alpha.150.tar.gz | openssl base64
```
...
- Useful-view and saved-collection count badges now use a count-only catalogue path.
- Normal count-only queries no longer fetch item rows or catalogue facets.
- Scanner-conflict counts preserve the existing visible-catalogue semantics; SQL-only conflict counting remains pending.
