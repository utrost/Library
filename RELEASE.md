...
This repo is prepared for the v0.1 manual testing pass from app version `0.1.0-alpha.151`. The existing public prerelease tag is `v0.1.0-alpha.1`; create a new tag only after the manual test pass is accepted.

## Release type

- Private/early-tester v0.1 candidate on a Nextcloud 34 test instance.
- App version in `appinfo/info.xml`: `0.1.0-alpha.151`.
...
```text
dist/library-0.1.0-alpha.151.tar.gz
dist/library-0.1.0-alpha.151.tar.gz.sha256
```
...
```bash
openssl dgst -sha512 -sign ~/.nextcloud/certificates/library.key dist/library-0.1.0-alpha.151.tar.gz | openssl base64
```
...
- Ordinary catalogue/AJAX item DTOs omit unbounded cover override blobs, raw provenance maps, comments and detail-only mutation URLs while retaining tags, descriptions, diagnostics and visible card actions.
- Normal catalogue SQL avoids selecting cover override data; scanner-conflict and weak-metadata review views retain rich provenance.
- Detail, cover, export and import remain full-fidelity; creator landing URL duplication, description/lazy detail loading, indexes, unchanged rescans, the saved raw-tag bug and SQL conflict counting remain pending.
